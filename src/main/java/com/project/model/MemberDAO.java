...
public class MemberDAO {
	...
	public MemberDO selectedByMember_id(int member_id) {
		...
		this.sql = "select member_id, m_name, m_acctid, m_acctpwd, m_email, m_tel, m_dept, m_pfp, m_status, m_role "
				+ "from final_member " + "where member_id=?";
	...
	public MemberDO login(String m_acctid) {
		MemberDO member = null;
		this.sql = "select member_id, m_name, m_acctid, m_acctpwd, m_email, m_tel, m_dept, m_pfp, m_status, m_role "
				+ "from final_member " + "where m_acctid=?";
	...
	public int insertMember(MemberDO member) {
		...
		try {
			result = this.jdbcTemplate.update(sql, member.getM_acctid(), member.getM_acctpwd(), member.getM_name(),
					member.getM_email(), member.getM_tel(), member.getM_dept(), member.getM_status(),
					member.getM_role());
	...
	public int updateEmail(MemberDO member) {
		this.sql = "update final_member " + "set m_email=?" + "where member_id=?";
		return this.jdbcTemplate.update(sql, member.getM_email(), member.getMember_id());
	}

	public int updatePassword(MemberDO member) {
		this.sql = "update final_member " + "set m_acctpwd=? " + "where member_id=?";
		return this.jdbcTemplate.update(sql, member.getM_acctpwd(), member.getMember_id());
	}

	public int updateMemberStatus(MemberDO member) {
		this.sql = "update final_member " + "set m_status=?, m_deactivation_date = ? " + "where member_id=?";
		Timestamp timestamp = (member.getDeactivationDate() != null) ? Timestamp.valueOf(member.getDeactivationDate())
				: null;
	...
	public int updateMemberProfileImg(MemberDO member) {
		this.sql = "update final_member " + "set m_pfp=?" + "where member_id=?";
